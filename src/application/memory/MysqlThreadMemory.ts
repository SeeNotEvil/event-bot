...
import { AccessDeniedError, type BotAccess } from '../access/BotAccess.js';
...
export class MysqlThreadMemory implements ThreadMemory {
  ...
  public async ensure(chatId: number) {
    ...
    if (this.access) {
      const key = await this.access.summaryKey();
      // Invalidate the lease too: a summary generated with old membership cannot be committed.
      await this.database.updateTable('threads').set({ summary: null, summary_cursor: 0,
        summary_access_key: key, summary_version: sql<number>`summary_version + 1`,
        lock_token: null, locked_at: null, retry_at: null, last_error: null })
        .where('chat_id', '=', chatId)
        .where((eb) => eb.or([eb('summary_access_key', 'is', null), eb('summary_access_key', '!=', key)])).execute();
    }
  ...
  public async append(threadId: number, userId: number | null, rawMessage: ConversationMessage, metadata: MessageMetadata = {}) {
    ...
    return this.database.transaction().execute(async (transaction) => {
      if (this.access && !await transaction.selectFrom('users').select('id').where('id', '=', userId ?? 0)
        .where(this.access.allowsUser(sql.ref('users.id'))).executeTakeFirst()) throw new AccessDeniedError();
  ...
  public async read(threadId: number, limit: number, beforeId?: number, excludeMessageId?: number) {
    ...
    if (this.access) query = query.where(this.access.allowsUser(sql.ref('conversation_messages.user_id')));
  ...
  public async claimSummary(recentLimit: number, leaseMs: number): Promise<SummaryBatch | null> {
    const accessKey = await this.access?.summaryKey();
    const currentAccess = accessKey === undefined ? sql<boolean>`true`
      : sql<boolean>`coalesce(threads.summary_access_key = ${accessKey}, false)`;
    return this.database.transaction().execute(async (transaction) => {
      const now = new Date();
      const row = await transaction.selectFrom('threads').selectAll()
        .where((eb) => eb.or([eb.not(currentAccess), eb('locked_at', 'is', null), eb('locked_at', '<', new Date(now.getTime() - leaseMs).toISOString().slice(0, 23).replace('T', ' '))]))
        .where((eb) => eb.or([eb.not(currentAccess), eb('retry_at', 'is', null), eb('retry_at', '<=', now.toISOString().slice(0, 23).replace('T', ' '))]))
        .where(sql<boolean>`exists (select 1 from conversation_messages m
          where m.thread_id = threads.id and m.id > (case when ${currentAccess} then threads.summary_cursor else 0 end)
          and ${this.access ? this.access.allowsUser(sql.ref('m.user_id')) : sql<boolean>`true`}
      ...
      const sameAccess = accessKey === undefined || row.summary_access_key === accessKey;
      const summary = sameAccess ? row.summary : null;
      const summaryCursor = sameAccess ? Number(row.summary_cursor) : 0;
      const summaryVersion = Number(row.summary_version) + (sameAccess ? 0 : 1);
      let query = transaction.selectFrom('conversation_messages').selectAll()
        .where('thread_id', '=', row.id).where('id', '>', summaryCursor)
        .orderBy('id').limit(SUMMARY_BATCH_SIZE);
      if (this.access) query = query.where(this.access.allowsUser(sql.ref('conversation_messages.user_id')));
      const messages = await query.execute();
      await transaction.updateTable('threads').set({ lock_token: token, locked_at: now,
        summary, summary_cursor: summaryCursor, summary_version: summaryVersion,
        summary_access_key: accessKey ?? row.summary_access_key, retry_at: null, last_error: null })
        .where('id', '=', row.id).execute();
      return { id: Number(row.id), summary, summaryCursor,
        summaryVersion, token, messages: messages.map(mapMessage), accessKey };
    });
  }

  public async completeSummary(batch: SummaryBatch, rawSummary: string) {
    if (this.access && batch.accessKey !== await this.access.summaryKey()) return false;
