...
import type { AccessPolicy } from '../../application/access/BotAccess.js';

export function createSearchChatMembersTool(database: Kysely<Database>, telegram: TelegramMembershipGateway, access?: AccessPolicy) {
  return defineTool({
    ...
    execute: async (context, input) => {
      ...
      if (access) {
        const allowed = await Promise.all(result.members.map((member) => access.isAllowed(member.telegramUserId)));
        result.members = result.members.filter((_member, index) => allowed[index]);
      }
...
export function createSetReminderRecipientTool(database: Kysely<Database>, telegram: TelegramMembershipGateway, access?: AccessPolicy) {
  return defineTool({
    ...
    execute: (context, input) => setReminderRecipient(database, telegram, context, input, access),
  });
}
