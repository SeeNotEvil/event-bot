...
export function createSaveUserPreferencesTool(database: Kysely<Database>) {
  return defineTool({
    ...
    description:
      'Сохраняет цельную актуальную памятку предпочтений текущего пользователя. Используй только для явно высказанных устойчивых пожеланий или просьбы запомнить: обращение, стиль общения и привычки работы с расписанием. Передавай краткую нормализованную выжимку целиком, сохраняя неизменившиеся предпочтения из context и удаляя исправленные; не копируй исходные команды. Не делай выводов по косвенным признакам и не сохраняй пароли, API-ключи или платёжные данные. preferences=null означает забыть все сохранённые предпочтения.',
