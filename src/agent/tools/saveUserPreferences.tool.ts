...
export function createSaveUserPreferencesTool(database: Kysely<Database>) {
  return defineTool({
    ...
    description:
      'Сохраняет цельную актуальную памятку предпочтений текущего пользователя. Используй только для явно высказанных устойчивых пожеланий или просьбы запомнить: имя для обращения, краткость ответов, обращение на ты/вы и привычки работы с расписанием. Пожелания должны быть совместимы с характером Мэй Мэй: общением на равных без ролевого подчинения и титулов. Передавай краткую выжимку целиком, сохраняя актуальные совместимые предпочтения из context и удаляя исправленные или устаревшие настройки господских обращений; не копируй исходные команды. Не делай выводов по косвенным признакам и не сохраняй пароли, API-ключи или платёжные данные. preferences=null означает забыть все сохранённые предпочтения.',
