...
import { createNote, deleteNote, noteSchema, readNote, searchNotes, updateNote } from '../../src/application/notes/notes.js';
...
import { createNoteTools } from '../../src/agent/tools/note.tools.js';
...
describeWithMysql('MySQL application actions', () => {
  ...
  it('isolates personal notebooks across chats and protects searches, edits and deletion by owner and version', async () => {
    ...
    const access = new BotAccess(database, author.telegramUserId);
    const registry = new ToolRegistry();
    for (const tool of createNoteTools(database)) registry.register(tool as AnyTool);
    const runtime = new ToolRuntime(registry, silentLogger, access);
    const groupContext = { userId: author.id, telegramUserId: author.telegramUserId,
      chatId: group.id, chatType: 'group' } as AgentContext;
    const personalContext = { ...groupContext, chatId: personal.id, chatType: 'personal' as const };
    const memberContext = { ...groupContext, userId: member.id, telegramUserId: member.telegramUserId };
    try {
      await access.allow(author.telegramUserId, member.telegramUserId);
      ...
      const groupNote = await createNote(database, group.id, author.id, original);
      expect(groupNote.tags).toEqual(['поездка', 'сборы']);
      expect(await runtime.execute('read_note', { id: privateNote.id }, groupContext))
        .toMatchObject({ ok: true, output: { note: privateNote } });
      expect(await runtime.execute('read_note', { id: groupNote.id }, personalContext))
        .toMatchObject({ ok: true, output: { note: groupNote } });
      for (const context of [memberContext, { ...memberContext, chatId: otherPersonal.id, chatType: 'personal' as const }]) {
        expect(await runtime.execute('read_note', { id: privateNote.id }, context)).toMatchObject({ ok: true, output: { note: null } });
        expect(await runtime.execute('read_note', { id: groupNote.id }, context)).toMatchObject({ ok: true, output: { note: null } });
        expect(await runtime.execute('search_notes', input, context)).toMatchObject({ ok: true, output: { notes: [], nextBeforeId: null } });
        for (const expectedVersion of [1, 999]) {
          expect(await runtime.execute('update_note', { ...original, id: groupNote.id, expectedVersion }, context))
            .toMatchObject({ ok: true, output: { success: false, reason: 'NOT_FOUND', note: null } });
          expect(await runtime.execute('delete_note', { id: groupNote.id, expectedVersion }, context))
            .toMatchObject({ ok: true, output: { success: false, reason: 'NOT_FOUND', note: null } });
        }
      }
      expect(await updateNote(database, member.id, { ...original, id: privateNote.id, expectedVersion: 1 }))
        .toMatchObject({ success: false, reason: 'NOT_FOUND', note: null });
      expect(await deleteNote(database, member.id, { id: privateNote.id, expectedVersion: 1 }))
        .toMatchObject({ success: false, reason: 'NOT_FOUND', note: null });
      // The literal search term is beyond the excerpt and contains LIKE metacharacters.
      const found = await searchNotes(database, author.id, { ...input, query: '50%_!', tag: ' ПОЕЗДКА ' });
      expect(found.notes.map((note) => note.id)).toEqual([groupNote.id]);
      ...
      expect((await searchNotes(database, author.id, { ...input, query: 'Поездка' })).notes.map((note) => note.id)).toEqual([groupNote.id]);
      expect((await searchNotes(database, author.id, { ...input, tag: 'поезд' })).notes).toEqual([]);
      expect((await readNote(database, author.id, { id: groupNote.id }))?.content).toBe(original.content);

      const created = await runtime.execute('create_note', { title: 'Покупки: скидка 50abc!', content: '', tags: [],
        userId: member.id, createdByUserId: member.id, chatId: otherPersonal.id }, groupContext);
      expect(created).toMatchObject({ ok: true, output: { createdByUserId: author.id } });
      const second = noteSchema.parse(created.output);
      const foreign = await createNote(database, group.id, member.id, original);
      // Even the bot owner cannot access another member's notebook.
      expect(await runtime.execute('read_note', { id: foreign.id }, groupContext)).toMatchObject({ ok: true, output: { note: null } });
      expect(await runtime.execute('update_note', { ...original, id: foreign.id, expectedVersion: 1 }, groupContext))
        .toMatchObject({ ok: true, output: { success: false, reason: 'NOT_FOUND', note: null } });
      expect(await runtime.execute('delete_note', { id: foreign.id, expectedVersion: 1 }, groupContext))
        .toMatchObject({ ok: true, output: { success: false, reason: 'NOT_FOUND', note: null } });
      for (const context of [personalContext, groupContext]) {
        expect(await runtime.execute('search_notes', input, context)).toMatchObject({ ok: true,
          output: { notes: [{ id: second.id }, { id: groupNote.id }, { id: privateNote.id }], nextBeforeId: null } });
      }
      expect(await runtime.execute('search_notes', input, memberContext)).toMatchObject({ ok: true,
        output: { notes: [{ id: foreign.id }], nextBeforeId: null } });
      expect((await searchNotes(database, author.id, { ...input, query: '50%_!' })).notes.map((note) => note.id)).toEqual([groupNote.id]);
      const page = await searchNotes(database, author.id, { ...input, limit: 1 });
      expect(page.notes.map((note) => note.id)).toEqual([second.id]);
      const next = await searchNotes(database, author.id, { ...input, limit: 1, beforeId: page.nextBeforeId });
      expect(next.notes.map((note) => note.id)).toEqual([groupNote.id]);
      const last = await searchNotes(database, author.id, { ...input, limit: 1, beforeId: next.nextBeforeId });
      expect(last.notes.map((note) => note.id)).toEqual([privateNote.id]);
      expect(last.nextBeforeId).toBeNull();

      const updates = await Promise.all([
        updateNote(database, author.id, { ...original, id: groupNote.id, expectedVersion: 1, content: `${original.content}\n- [ ] Билеты` }),
        updateNote(database, author.id, { ...original, id: groupNote.id, expectedVersion: 1, content: `${original.content}\n- [ ] Наушники` }),
      ]);
      ...
      expect(winner).toMatchObject({ createdByUserId: author.id, createdAtUtc: groupNote.createdAtUtc, version: 2 });
      expect(await deleteNote(database, author.id, { id: groupNote.id, expectedVersion: 1 }))
        .toMatchObject({ success: false, reason: 'VERSION_CONFLICT', note: winner });
      expect(await runtime.execute('update_note', { id: groupNote.id, expectedVersion: 2,
        title: 'Сборы в поездку', content: winner.content.replace('- [ ] Паспорт', '- [x] Паспорт'), tags: [] }, personalContext))
        .toMatchObject({ ok: true, output: { success: true, note: { title: 'Сборы в поездку', tags: [], version: 3 } } });
      const edited = await readNote(database, author.id, { id: groupNote.id });
      expect(edited?.content).toContain('- [x] Паспорт');
      expect(edited?.content).toContain('зарядка');
      expect((await searchNotes(database, author.id, { ...input, tag: 'поездка' })).notes).toEqual([]);
      expect(await runtime.execute('delete_note', { id: groupNote.id, expectedVersion: 3 }, personalContext))
        .toMatchObject({ ok: true, output: { success: true, note: null } });
      expect(await readNote(database, author.id, { id: groupNote.id })).toBeNull();
      expect((await searchNotes(database, author.id, input)).notes.map((note) => note.id)).toEqual([second.id, privateNote.id]);
      expect((await database.selectFrom('notes').select('chat_id').where('id', '=', second.id).executeTakeFirstOrThrow()).chat_id).toBe(group.id);
    } finally {
      await database.deleteFrom('bot_access').where('telegram_user_id', '=', member.telegramUserId).execute();
  ...
  it('runs a persisted task as its owner and can schedule from background and finish without sending', async () => {
    ...
    try {
      ...
      const notes = await searchNotes(database, user.id, { query: null, tag: null, beforeId: null, limit: null });
  ...
  it('summarizes successive archive batches without skipping messages after failures or a replaced lease', async () => {
    ...
    try {
      for (let index = 0; index < 24; index++) await threads.append(thread.id, null, { role: 'user', content: `message-${index}` });
      expect(await threads.claimSummary(15, 60_000)).toBeNull();
      await threads.append(thread.id, null, { role: 'user', content: 'message-24' });
      const first = (await threads.claimSummary(15, 60_000))!;
      expect(first.messages).toHaveLength(10);
      expect(first.messages.map((message) => message.content)).toEqual(Array.from({ length: 10 }, (_, i) => `message-${i}`));
      expect(await threads.claimSummary(15, 60_000)).toBeNull();
      ...
      const retry = (await threads.claimSummary(15, 60_000))!;
      ...
      const replacement = (await threads.claimSummary(15, 60_000))!;
      ...
      expect(await threads.claimSummary(15, 60_000)).toBeNull();
      for (let index = 25; index < 35; index++) await threads.append(thread.id, null, { role: 'user', content: `message-${index}` });
      ...
      const older = await threads.read(thread.id, 10, recent[0]!.id);
      expect(older.map((message) => message.id)).toEqual(second.messages.map((message) => message.id));
      expect(await threads.read(thread.id, 200)).toHaveLength(35);
      expect(await threads.claimSummary(15, 60_000)).toBeNull();
    } finally {
      await database.deleteFrom('chats').where('id', '=', chat.id).execute();
    }
